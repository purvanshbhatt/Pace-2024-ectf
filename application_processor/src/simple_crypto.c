...
int import_pub_key(const byte* pubkey_buf, word32 len, ecc_key* key)
{
	...
	if (import_result != 0) {
		return import_result; //Report error
	}
 ...
 * @return 0 on success, non-zero for other error
 */
int asym_sign(uint8_t *ciphertext, byte *signature, ecc_key *key, word32 *sig_len, WC_RNG *rng) {
 ...
 *
 * @param ciphertext	Pointer to buffer storing the encrypted command to hash for signature validation
 *
...
int asym_validate(const byte *signature, word32 sig_len, const byte *ciphertext, int *status, ecc_key *key) {
    ...
    //Validate Signature
    int valid = wc_ecc_verify_hash(signature, sig_len, digest, HASH_SIZE, status, key);
    if (valid != 0) {
